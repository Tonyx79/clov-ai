...
This ensures CLOV **never returns false data silently** while maintaining maximum token efficiency.
        ...
        ParseResult::Degraded(data, warnings) => {
            if verbose > 0 {
                for warn in warnings {
                    eprintln!("[CLOV:DEGRADED] {}", warn);
                }
            }
            data.format(mode)
        }
        ParseResult::Passthrough(raw) => {
            eprintln!("[CLOV:PASSTHROUGH] Parser failed, showing truncated output");
            raw
        }
    };
...
```
[CLOV:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[CLOV:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
