...
//!
//! The three-tier system ensures CLOV never returns false data silently.
...
pub trait OutputParser: Sized {
    ...
    /// 3. Return truncated passthrough with `[CLOV:PASSTHROUGH]` marker
...
pub fn truncate_output(output: &str, max_chars: usize) -> String {
    ...
    format!(
        "{}\n\n[CLOV:PASSTHROUGH] Output truncated ({} chars → {} chars)",
        truncated,
        chars.len(),
        max_chars
    )
...
pub fn emit_degradation_warning(tool: &str, reason: &str) {
    eprintln!("[CLOV:DEGRADED] {} parser: {}", tool, reason);
}

/// Helper to emit passthrough warning
pub fn emit_passthrough_warning(tool: &str, reason: &str) {
    eprintln!("[CLOV:PASSTHROUGH] {} parser: {}", tool, reason);
}
...
mod tests {
    ...
    fn test_truncate_output() {
        ...
        assert!(truncated.contains("[CLOV:PASSTHROUGH]"));
    ...
    fn test_truncate_output_multibyte() {
        ...
        assert!(result.contains("[CLOV:PASSTHROUGH]"));
