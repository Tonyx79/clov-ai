...
fn run_generate(args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track("prisma generate", "clov prisma generate", &raw, &filtered);
...
fn run_migrate(subcommand: MigrateSubcommand, args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track(cmd_name, &format!("clov {}", cmd_name), &raw, &filtered);
...
fn run_db_push(args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track("prisma db push", "clov prisma db push", &raw, &filtered);
