...
fn docker_ps(_verbose: u8) -> Result<()> {
    ...
    if stdout.trim().is_empty() {
        clov.push_str("🐳 0 containers");
        println!("{}", clov);
        timer.track("docker ps", "clov docker ps", &raw, &clov);
        return Ok(());
    }

    let count = stdout.lines().count();
    clov.push_str(&format!("🐳 {} containers:\n", count));
    ...
    if count > 15 {
        clov.push_str(&format!("  ... +{} more", count - 15));
    }
...
fn docker_images(_verbose: u8) -> Result<()> {
    ...
    if lines.is_empty() {
        clov.push_str("🐳 0 images");
        println!("{}", clov);
        timer.track("docker images", "clov docker images", &raw, &clov);
        return Ok(());
    }
    ...
    if lines.len() > 15 {
        clov.push_str(&format!("  ... +{} more", lines.len() - 15));
    }
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if container.is_empty() {
        println!("Usage: clov docker logs <container>");
        return Ok(());
    }
...
fn kubectl_pods(args: &[String], _verbose: u8) -> Result<()> {
    ...
    let json: serde_json::Value = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(_) => {
            clov.push_str("☸️  No pods found");
            println!("{}", clov);
            timer.track("kubectl get pods", "clov kubectl pods", &raw, &clov);
            return Ok(());
        }
    };
    ...
    clov.push_str(&format!("☸️  {} pods: {}\n", pods.len(), parts.join(", ")));
    if !issues.is_empty() {
        clov.push_str("⚠️  Issues:\n");
        for issue in issues.iter().take(10) {
            clov.push_str(&format!("  {}\n", issue));
        }
        if issues.len() > 10 {
            clov.push_str(&format!("  ... +{} more", issues.len() - 10));
        }
    }
...
fn kubectl_services(args: &[String], _verbose: u8) -> Result<()> {
    ...
    let json: serde_json::Value = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(_) => {
            clov.push_str("☸️  No services found");
            println!("{}", clov);
            timer.track("kubectl get svc", "clov kubectl svc", &raw, &clov);
            return Ok(());
        }
    };
    ...
    clov.push_str(&format!("☸️  {} services:\n", services.len()));
    ...
    if services.len() > 15 {
        clov.push_str(&format!("  ... +{} more", services.len() - 15));
    }
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if pod.is_empty() {
        println!("Usage: clov kubectl logs <pod>");
        return Ok(());
    }
...
pub fn run_docker_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("clov docker {} (passthrough)", args_str),
    );
...
pub fn run_compose_ps(verbose: u8) -> Result<()> {
    ...
    let clov = format_compose_ps(&structured);
    println!("{}", clov);
    timer.track("docker compose ps", "clov docker compose ps", &raw, &clov);
    Ok(())
}
...
pub fn run_compose_build(service: Option<&str>, verbose: u8) -> Result<()> {
    ...
    let clov = format_compose_build(&raw);
    println!("{}", clov);
    let svc_label = service.unwrap_or("all");
    timer.track(
...
pub fn run_compose_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("clov docker compose {} (passthrough)", args_str),
    );
...
pub fn run_kubectl_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("clov kubectl {} (passthrough)", args_str),
    );
