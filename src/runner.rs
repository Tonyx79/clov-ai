...
pub fn run_err(command: &str, verbose: u8) -> Result<()> {
    ...
    if filtered.is_empty() {
        if output.status.success() {
            clov.push_str("✅ Command completed successfully (no errors)");
        } else {
    ...
    timer.track(command, "clov run-err", &raw, &clov);
    Ok(())
}
...
pub fn run_test(command: &str, verbose: u8) -> Result<()> {
    ...
    timer.track(command, "clov run-test", &raw, &summary);
    Ok(())
}
