...
fn get_tee_dir(config: &Config) -> Option<PathBuf> {
    // Env var override
    if let Ok(dir) = std::env::var("CLOV_TEE_DIR") {
    ...
    // Default: ~/.local/share/clov/tee/
    dirs::data_local_dir().map(|d| d.join("clov").join("tee"))
}
...
pub fn tee_raw(raw: &str, command_slug: &str, exit_code: i32) -> Option<PathBuf> {
    // Check CLOV_TEE=0 env override (disable)
    if std::env::var("CLOV_TEE").ok().as_deref() == Some("0") {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn test_format_hint() {
        let path = PathBuf::from("/tmp/clov/tee/123_cargo_test.log");
...
"#;
        ...
        assert_eq!(config.directory, Some(PathBuf::from("/tmp/clov-tee")));
