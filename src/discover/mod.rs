            ...
            for part in parts {
                ...
                match classify_command(part) {
                    Classification::Supported {
                        ...
                        let bucket = supported_map.entry(clov_equivalent).or_insert_with(|| {
                            SupportedBucket {
                                clov_equivalent,
                                category,
                                count: 0,
                    ...
                    Classification::Ignored => {
                        // Check if it starts with "clov "
                        if part.trim().starts_with("clov ") {
                            already_clov += 1;
                        }
                        // Otherwise just skip
                    }
                ...
                .map(|(name, _)| {
                    ...
                    if let Some(colon_pos) = name.rfind(':') {
                        ...
                        let status = match status_str {
                            "Passthrough" => report::ClovStatus::Passthrough,
                            "NotSupported" => report::ClovStatus::NotSupported,
                            _ => report::ClovStatus::Existing,
                        };
                        (cmd, status)
                    } else {
                        (name, report::ClovStatus::Existing)
                    }
                })
                .unwrap_or_else(|| (String::new(), report::ClovStatus::Existing));

            SupportedEntry {
                command: command_with_status,
                count: bucket.count,
                clov_equivalent: bucket.clov_equivalent,
