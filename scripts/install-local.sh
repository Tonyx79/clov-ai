#!/bin/bash
# Install CLOV from a local release build (builds from source, no network download).

set -euo pipefail

INSTALL_DIR="${1:-$HOME/.cargo/bin}"
INSTALL_PATH="${INSTALL_DIR}/clov"
BINARY_PATH="./target/release/clov"
