...
Comprehensive testing rules for CLOV CLI tool development.

## Snapshot Testing (🔴 Critical)

**Priority**: 🔴 **Triggers**: All filter changes, output format modifications

Use `insta` crate for output validation. This is the **primary testing strategy** for CLOV filters.
...
CLOV targets <10ms startup time and <5MB memory usage.
...
# Benchmark CLOV vs raw command
hyperfine 'clov git status' 'git status' --warmup 3

# Should show CLOV startup <10ms
# Example output:
#   clov git status    6.2 ms ±  0.3 ms
...
```bash
# macOS
/usr/bin/time -l clov git status
# Look for "maximum resident set size" - should be <5MB

# Linux
/usr/bin/time -v clov git status
...
|--------|--------|--------------|
| Startup time | <10ms | `hyperfine 'clov <cmd>'` |
| Memory usage | <5MB | `time -l clov <cmd>` |
| Binary size | <5MB | `ls -lh target/release/clov` |
...
hyperfine 'clov cmd' --warmup 3 > /tmp/before.txt
# Make changes
cargo build --release
hyperfine 'target/release/clov cmd' --warmup 3 > /tmp/after.txt
diff /tmp/before.txt /tmp/after.txt
```
