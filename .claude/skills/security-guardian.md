---
description: CLI security expert for CLOV - command injection, shell escaping, hook security
---

# Security Guardian

Comprehensive security analysis for CLOV CLI tool, focusing on **command injection**, **shell escaping**, **hook security**, and **malicious input handling**.
...
## CLOV Security Threat Model

CLOV faces unique security challenges as a CLI proxy that:
1. **Executes shell commands** based on user input
2. **Parses untrusted command output** (git, cargo, gh, etc.)
3. **Integrates with Claude Code hooks** (clov-rewrite.sh, clov-suggest.sh)
...
| **Malicious Output** | 🟡 MEDIUM | CLOV crash, DoS | Robust parsing, error handling |
...
# Validate CLOV binary exists and is executable
if ! command -v clov >/dev/null 2>&1; then
    echo "Error: clov binary not found"
    exit 1
fi
...
fn test_malicious_output_handling() {
    // Malformed outputs that could crash CLOV
...
# 4. Validate CLOV version (prevent downgrade attacks)
if ! "$CLOV_BIN" --version | grep -q "clov 0.16"; then
    echo "Warning: CLOV version mismatch"
fi
...
```markdown
## Security Advisory: Command Injection in clov v0.16.0
...
**Description**:
CLOV versions 0.15.0 through 0.16.0 are vulnerable to command injection
...
**Mitigation**:
Upgrade to v0.16.1 immediately. As a workaround, avoid using CLOV in
